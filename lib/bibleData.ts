...
const TRANSLATION_OMITTED_VERSES: Record<number, Record<string, Record<number, readonly number[]>>> = {
  92: {
    "사도행전": { 24: [7] },
  },
  98: {
...
export function getBibleChapterMaxVerse(
  koreanBookName: string,
  chapter: number | string,
  translationId: number,
) {
  const chapterNumber = Number(chapter);
  if (!Number.isSafeInteger(chapterNumber) || chapterNumber < 1) return 176;

  return (
    TRANSLATION_MAX_VERSE_OVERRIDES[translationId]?.[koreanBookName]?.[chapterNumber] ??
    BIBLE_CHAPTERS[koreanBookName]?.[chapterNumber - 1] ??
    176
  );
}

export function getBibleVerseNumbers(
  koreanBookName: string,
  chapter: number | string,
  translationId: number,
) {
  const chapterNumber = Number(chapter);
  const maxVerse = getBibleChapterMaxVerse(koreanBookName, chapterNumber, translationId);
  const omitted = new Set(
    TRANSLATION_OMITTED_VERSES[translationId]?.[koreanBookName]?.[chapterNumber] ?? [],
  );

  return Array.from({ length: maxVerse }, (_, index) => index + 1)
    .filter((verse) => !omitted.has(verse));
}
