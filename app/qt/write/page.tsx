...
const VERSIONS = ["개역한글","개역개정","쉬운성경","KJV","NIV"];
...
const STEPS = [
  ...
  { id: "bible_ref", step: 2, title: "본문 & 요약", subtitle: "성경 본문을 선택하고 묵상해요", placeholder: "", hint: "본문을 읽고 내용을 간략히 요약해보세요.", isPassage: true },
...
// 성경 본문 선택 UI
function BibleSelector({ version, onPassageLoad }: { version: string; onPassageLoad: (text: string, ref: string) => void }) {
  ...
  async function loadPassage() {
    ...
    try {
      ...
      if (data.error) {
        ...
        onPassageLoad(data.text, data.reference);
      }
    } catch {
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 50, display: "flex", alignItems: "flex-end" }}>
          <div style={{ background: "var(--bg2)", width: "100%", maxWidth: 430, margin: "0 auto", borderRadius: "24px 24px 0 0", padding: "20px 0", maxHeight: "70vh", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "0 20px 14px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setShowBookPicker(false)} style={{ background: "none", border: "none", color: "var(--text3)", cursor: "pointer", fontSize: 18 }}>✕</button>
            </div>
...
function QTWriteContent() {
  ...
  function handlePassageLoad(text: string, ref: string) {
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      {step.isPassage ? (
        <div style={{ flex: 1, padding: "16px 16px 0", overflowY: "auto", display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ background: "var(--bg2)", borderRadius: 16, padding: "12px 14px", border: "1px solid var(--border)" }}>
            <p style={{ fontSize: 11, fontWeight: 600, color: "var(--text3)", marginBottom: 4 }}>선택된 버전</p>
            <p style={{ fontSize: 14, fontWeight: 600, color: "var(--sage-dark)" }}>{initVersion}</p>
          </div>

          <BibleSelector version={initVersion} onPassageLoad={handlePassageLoad} />

          {/* 본문 불러온 후 표시 */}
          {passageLoaded && answers.passage && (
            <div style={{ background: "var(--sage-light)", borderRadius: 14, padding: "14px 16px", border: "1px solid rgba(122,157,122,0.3)" }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--sage-dark)", marginBottom: 8 }}>{answers.bible_ref}</p>
              <p style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.7, fontStyle: "italic" }}>{answers.passage}</p>
              <button onClick={() => { setPassageLoaded(false); setAnswers(p => ({ ...p, passage: "", bible_ref: "" })); }} style={{ marginTop: 8, fontSize: 11, color: "var(--text3)", background: "none", border: "none", cursor: "pointer" }}>
                다시 선택하기
              </button>
            </div>
          )}
          ...
          {passageLoaded && (
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: "var(--text3)", display: "block", marginBottom: 6 }}>붙잡은 말씀 <span style={{ fontWeight: 400 }}>(위 본문에서 와닿은 구절 복붙)</span></label>
        ...
        <div style={{ flex: 1, padding: "16px 16px 0", display: "flex", flexDirection: "column", gap: 10, overflowY: "auto" }}>
          {cur >= 2 && answers.passage && (
            <div style={{ background: "var(--sage-light)", borderRadius: 14, padding: "10px 14px" }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "var(--sage-dark)", marginBottom: 6 }}>{answers.bible_ref}</p>
              <p style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6, fontStyle: "italic" }}>{answers.passage.slice(0, 100)}{answers.passage.length > 100 ? "..." : ""}</p>
            </div>
          )}
