...
import { ChevronLeft, Check, Loader2, Plus, Trash2, ChevronDown, BookOpen, X } from "lucide-react";
...
const STEPS_6 = [
  ...
  { id: "meditation", step: 2, title: "느낌과 묵상", subtitle: "이 말씀이 내게 주는 의미", placeholder: "이 말씀이 오늘 내 삶에 무슨 말씀인가요?\n솔직하게 느낀 것을 써보세요.", hint: "정답이 없어요. 성령님의 이끄심에 맡겨봐요." },
  { id: "application", step: 3, title: "적용과 결단", subtitle: "오늘 하루 어떻게 살 건가요?", placeholder: "", hint: "성품은 마음을 정하는 것, 행동은 손과 발로 드러나는 것이에요.", isDecision: true },
  { id: "closing_prayer", step: 4, title: "올려드리는 기도", subtitle: "말씀으로 드리는 기도", placeholder: "말씀을 붙들고 기도를 올려드려요...", hint: "말씀과 결단을 간결하게 다시 하나님께 올려드려요.", isLast: true },
];

const STEPS_SUNDAY = [
  { id: "summary", step: 1, title: "설교 요약", subtitle: "설교 말씀을 내 말로 요약해요", placeholder: "오늘 설교 제목, 본문, 핵심 내용을 요약해보세요...", hint: "설교자가 전한 핵심 메시지를 나의 말로 정리해요." },
  { id: "key_verse", step: 2, title: "붙잡은 말씀", subtitle: "마음에 남은 구절이나 문장", placeholder: "설교 중 마음에 꽂힌 말씀이나 문장을 적어보세요...", hint: "말씀이 당신의 마음 밭에서 멈춘 곳을 붙잡으세요." },
  { id: "meditation", step: 3, title: "느낌과 묵상", subtitle: "하나님이 내게 하신 말씀", placeholder: "오늘 설교를 통해 하나님이 나에게 하신 말씀은 무엇인가요?", hint: "개인적이고 솔직하게 써보세요." },
  { id: "application", step: 4, title: "적용과 결단", subtitle: "이번 주 어떻게 살 건가요?", placeholder: "", hint: "말씀이 내 성품과 삶이 되도록 의지적으로 결단해요.", isDecision: true },
  { id: "closing_prayer", step: 5, title: "올려드리는 기도", subtitle: "예배의 마무리 기도", placeholder: "오늘 받은 은혜에 감사하며...", hint: "받은 말씀과 결단을 하나님께 올려드려요.", isLast: true },
];
...
function QTWriteContent() {
  ...
  // 번역본
  const [translations, setTranslations] = useState<{ id: number; name: string }[]>([]);
  const [selectedTranslation, setSelectedTranslation] = useState<number>(92); // 개역개정
    ...
    fetch("/api/translations")
      ...
      .then(json => {
        if (json.ok && json.data) {
          const korean = json.data.filter((t: any) => t.language === "ko" || t.language === "kor");
          setTranslations(korean.length > 0 ? korean : json.data.slice(0, 10));
        }
      })
      .catch(() => {
        setTranslations([{ id: 92, name: "개역개정" }, { id: 1, name: "개역한글" }]);
      });
  }, []);
  ...
  async function save() {
    ...
    try {
      ...
      let insertData: any;
      if (mode === "free") {
        insertData = { ...baseData, meditation: freeText };
      } else {
        insertData = {
          ...baseData,
      ...
      if (error) {
        const { error: e2 } = await supabase.from("qt_records").insert({ ...insertData, qt_mode: undefined });
    ...
    return (
      <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
        <div style={{ background: "var(--bg)", padding: "56px 20px 16px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          <h1 style={{ fontSize: 20, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>오늘의 말씀 찾기</h1>
        ...
        <div style={{ flex: 1, padding: "16px 16px 100px", display: "flex", flexDirection: "column", gap: 12, overflowY: "auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            {[
              { label: "장", value: chapter, setter: setChapter, max: 150 },
              { label: "시작 절", value: startV, setter: setStartV, max: 176 },
              { label: "끝 절", value: endV, setter: setEndV, max: 176 },
              ...
              <div key={label}>
                ...
                <select value={value} onChange={e => setter(e.target.value)} className="input-field" style={{ padding: "12px 8px" }}>
                  {Array.from({ length: max }, (_, i) => String(i + 1)).map(v => (
                    <option key={v} value={v}>{v}{label === "장" ? "장" : "절"}</option>
                  ))}
                </select>
              </div>
            ))}
        ...
        {showTranslationPicker && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 50, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
            <div style={{ background: "var(--bg2)", width: "100%", maxWidth: 480, borderRadius: "24px 24px 0 0", padding: "24px 20px 40px", maxHeight: "60vh", overflowY: "auto" }}>
              ...
              {translations.length === 0 ? (
                <div style={{ display: "flex", justifyContent: "center", padding: 20 }}><Loader2 size={20} style={{ color: "var(--sage)" }} className="spin" /></div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {translations.map(t => (
                    <button key={t.id} onClick={() => { setSelectedTranslation(t.id); setShowTranslationPicker(false); }} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", borderRadius: 14, border: `1px solid ${selectedTranslation === t.id ? "var(--sage)" : "var(--border)"}`, background: selectedTranslation === t.id ? "var(--sage-light)" : "var(--bg3)", cursor: "pointer" }}>
                      <span style={{ fontSize: 14, fontWeight: 500, color: selectedTranslation === t.id ? "var(--sage-dark)" : "var(--text)" }}>{t.name}</span>
                      {selectedTranslation === t.id && <span style={{ fontSize: 12, color: "var(--sage)", fontWeight: 600 }}>✓</span>}
                    </button>
                  ))}
                </div>
              )}
  ...
  // ─── 말씀 로드 완료 후 표시 + 큐티 작성 ───
  const passageReady = bibleStep === "done" && passageVerses.length > 0;
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "var(--bg)", padding: "56px 20px 14px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        ...
        {passageReady && (
          <div style={{ background: "var(--sage-light)", borderRadius: 12, padding: "10px 14px", marginBottom: 10, border: "1px solid rgba(122,157,122,0.3)" }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "var(--sage-dark)", marginBottom: 4 }}>{bibleRef} · {translationName}</p>
            <p style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6, fontStyle: "italic" }}>
              {keyVerse ? `"${keyVerse.slice(0, 60)}${keyVerse.length > 60 ? "..." : ""}"` : passageVerses[0]?.text?.slice(0, 60) + "..."}
            </p>
          </div>
        )}
        ...
        {mode === "free" && <h1 style={{ fontSize: 20, fontWeight: 700, color: "var(--text)" }}>자유 큐티</h1>}
      </div>
      ...
      {mode !== "free" && (
        <div style={{ display: "flex", overflowX: "auto", background: "var(--bg2)", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          {STEPS.map((s, i) => {
            const done = isStepDone(i); const isCurr = i === cur;
            return (
      ...
      {/* 본문 — 말씀 선택 (6step의 붙잡은 말씀) */}
      {mode !== "free" && mode !== "sunday" && passageReady && cur === 0 && (
        <div style={{ padding: "12px 16px 0", background: "var(--bg)", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          ...
          {keyVerse && (
            <div style={{ marginTop: 8, marginBottom: 8 }}>
              <p style={{ fontSize: 10, fontWeight: 600, color: "var(--text3)", marginBottom: 4 }}>붙잡은 말씀</p>
              <textarea className="textarea-field" rows={2} value={keyVerse} onChange={e => setKeyVerse(e.target.value)} placeholder="마음에 와닿은 구절..." />
            </div>
          )}
        </div>
      )}
      ...
      {mode === "free" && (
        <div style={{ flex: 1, padding: "16px 16px 0", display: "flex", flexDirection: "column", gap: 14, overflowY: "auto" }}>
          {passageReady && (
            <div style={{ background: "var(--bg2)", borderRadius: 12, padding: "10px 14px", border: "1px solid var(--border)" }}>
              ...
              {keyVerse && <textarea className="textarea-field" rows={2} value={keyVerse} onChange={e => setKeyVerse(e.target.value)} placeholder="붙잡은 말씀..." />}
            </div>
          )}
          <div>
