...
export default function HomePage() {
  ...
  async function load() {
    ...
    const { data: ci } = await supabase.from("daily_checkins")
      .select("verse,reference,prayer_checked")
    ...
    setTodayDone({ qt: !!completedQt, prayer: !!tp || prayerChecked });
  ...
  useEffect(() => {
    ...
    if (profile.badge_angel) {
      const badgeKey = `badge_angel_${streak}`;
      if (!localStorage.getItem(badgeKey)) {
        localStorage.setItem(badgeKey, "true");
        setBadgePopup({ img: "/angel.png", title: t("badge_angel_title", lang), msg: t("badge_angel_desc", lang) });
        return;
      }
    }
