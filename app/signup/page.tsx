...
export default function SignupPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", padding: "56px 24px 40px" }}>
      ...
      <h1 style={{ fontSize: 26, fontWeight: 700, color: "var(--text)", marginBottom: 6 }}>시작하기</h1>
      <p style={{ color: "var(--text3)", fontSize: 13, marginBottom: 32 }}>오늘부터 말씀에 뿌리내려요 🌱</p>

      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div>
          <label style={{ color: "var(--text3)", fontSize: 12, display: "block", marginBottom: 6 }}>닉네임</label>
          <input type="text" value={nickname} onChange={e => setNickname(e.target.value)} placeholder="사용하실 닉네임을 입력해주세요" className="input-field" />
        </div>
        <div>
