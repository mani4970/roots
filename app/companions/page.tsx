...
function CompanionsContent() {
  ...
  const [inviteLang, setInviteLang] = useState<InviteLandingLang>(lang);
  ...
  const [removeTarget, setRemoveTarget] = useState<CompanionWithProfile | null>(
    null,
  );
  ...
  const inviteUrl = useMemo(
    () =>
      userId
        ? `${ROOTS_WEB_ORIGIN}/companions?invite=${userId}&lang=${lang}`
        : `${ROOTS_WEB_ORIGIN}/welcome?from=invite`,
    [userId, lang],
  );
  ...
  function currentCompanionsRedirectPath() {
    ...
    if (inviteUserId && !nextParams.get("lang"))
      nextParams.set("lang", inviteLang);
  ...
  function withProfiles(list: CompanionRow[]): CompanionWithProfile[] {
    return list.map((row) => ({
      ...row,
      other: profiles[otherId(row)] ?? null,
    }));
  }

  const accepted = withProfiles(
    rows.filter((row) => row.status === "accepted"),
  );
  const received = withProfiles(
    rows.filter(
      (row) => row.status === "pending" && row.receiver_id === userId,
    ),
  );
  const sent = withProfiles(
    rows.filter(
  ...
  async function loadAll() {
    ...
    if (!user) {
      ...
      router.push(
        `/welcome?redirect=${encodeURIComponent(currentCompanionsRedirectPath())}`,
      );
      return;
    }
    setUserId(user.id);
  ...
  useEffect(() => {
    ...
    const nextLang = resolveInviteLandingLang(langParam, lang, browserLanguage);
  ...
  function changeInviteLang(nextLang: InviteLandingLang) {
    ...
    router.replace(`/companions?${nextParams.toString()}`, { scroll: false });
  }
  ...
  async function updateRequest(
    ...
    try {
      ...
      showToast(
        status === "accepted"
          ? c("companions_request_accepted")
          : c("companions_request_declined"),
      );
      await loadAll();
    } catch (error) {
  ...
  async function deleteRelation(row: CompanionRow) {
    ...
    try {
      ...
      showToast(
        row.status === "accepted"
          ? c("companions_removed")
          : c("companions_request_cancelled"),
      );
      await loadAll();
    } catch (error) {
      ...
      >
        <div style={{ width: "100%", maxWidth: 380, textAlign: "center" }}>
          ...
          >
            ...
            >
              {inviteText.companionAfterAuthHint}
            </p>
            <div
              style={{
                height: 1,
            ...
            >
              {inviteText.companionAppDownloadHint}
            </p>
          </div>
        </div>
      </main>
    ...
    >
      ...
      {inviteUserId && inviteUserId !== userId && inviteProfile && (
        ...
        >
          ...
          >
            {c("companions_invite_from", {
              name: inviteProfile.name || "Roots",
            })}
          </h2>
          <p
            style={{
          ...
          >
            ...
            <button
              ...
              disabled={
                busyIds.includes(`invite:${inviteProfile.id}`) ||
                relationWith(inviteProfile.id)?.status === "accepted"
              }
              style={{
                padding: "10px 12px",
                borderRadius: 14,
            ...
            >
              ...
              {relationWith(inviteProfile.id)?.status === "accepted"
                ? c("companions_status_connected")
                : c("companions_accept_invite")}
            </button>
          </div>
        </div>
      )}
      ...
      {received.length === 0 ? (
        <EmptyCard text={c("companions_no_received_requests")} />
      ) : (
          ...
          {received.map((row) => (
            ...
            >
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <button
                  onClick={() => updateRequest(row, "accepted")}
                ...
                <button
                  onClick={() => updateRequest(row, "declined")}
                  disabled={busyIds.includes(row.id)}
                  style={{
                    width: 36,
                    height: 36,
      ...
      {accepted.length === 0 ? (
        <EmptyCard text={c("companions_empty")} />
      ) : (
        <div style={{ display: "grid", gap: 10 }}>
          {accepted.map((row) => (
            <div
      ...
      {sent.length === 0 ? (
        <EmptyCard text={c("companions_no_sent_requests")} />
      ) : (
        <div style={{ display: "grid", gap: 10 }}>
          {sent.map((row) => (
            <div
      ...
      {removeTarget && (
        ...
        >
          ...
          >
            ...
            >
              {c("companions_remove_confirm_title")}
            </h3>
            <p
              style={{
                fontSize: 13,
            ...
            >
              {c("companions_remove_confirm_body")}
            </p>
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => setRemoveTarget(null)}
              ...
              <button
                onClick={() => deleteRelation(removeTarget)}
                disabled={busyIds.includes(removeTarget.id)}
                style={{
                  flex: 1,
                  padding: "10px 12px",
                  borderRadius: 12,
              ...
              >
                {busyIds.includes(removeTarget.id) ? (
                  <Loader2 size={13} className="spin" />
                ) : (
                  c("companions_remove_confirm_button")
                )}
              </button>
            </div>
          </div>
